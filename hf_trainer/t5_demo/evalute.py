...
model_name_or_path = 'checkpoint-55136'
tokenizer = T5Tokenizer.from_pretrained(model_name_or_path)
model = T5ForConditionalGeneration.from_pretrained(model_name_or_path)
model.to("cuda")
for q in ('今天天气怎么样？', '今天中午吃什么？', '为什么会下雨', '黄晓明和angleboby什么时候结婚'):
    question = '自己添加一个prompt哦:' + q
...
with open('dev.jsonl.bak', 'r') as f:
    for index, line in enumerate(f, start=1):
        ...
        question = '自己添加一个prompt哦:' + data['input']
        inputs = tokenizer(question, return_tensors='pt').to('cuda')
        res = model.generate(
            **inputs,
            do_sample=True,
        ...
        infer_text = tokenizer.decode(res[0], skip_special_tokens=True)
...
# {'bleu1': 0.9691184171031797, 'bleu2': 0.9327564869738162, 'bleu3': 0.9048983105906694, 'bleu4': 0.8000254177928373, 'bleuAvg': 0.8710499019139355}
# 总样本:3418,正确:2854,Acc:0.8349912204944668
# {}
# 0.11892441576025234